...
while True:
	...
	if (normal_mode == False and duration_mode == False):
	    userInput = input("Would you like to enter a duration to monitor for? (Y/N)\n")
		
		if (userInput == "Y"):
			duration = input("Please enter a number of hours you wish to monitor for:\n")
			duration_mode = True
		else:
			print("Running normal monitoring protocols.")
			duration = -1
			normal_mode = True
	...
	if duration_mode == True and duration_set == False:
		end_time = take_reading_time + datetime.timedelta(hours=duration)
	...
	while (day_mode or duration_mode) and take_reading_time >= time_check and end_duration == False:
	    ...
	    if (time_check > end_time and duration_mode == True):
			end_duration = True
